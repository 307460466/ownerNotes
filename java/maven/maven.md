...
<a name="df6385f7"></a>
# setting.xml 介绍
> [参考](http://blog.csdn.net/tomato__/article/details/13025187)
...
```
  id，name：镜像的唯一标识和用户友好的名称；<br />  url：镜像的url，用于代替原始仓库的url；<br />  mirrorof：使用镜像的仓库的id，可以使用下面匹配属性：<br />           ------*：匹配所有仓库id；<br />           ------external:*：匹配所有仓库id，除了那些使用localhost或者基于仓库的文件的仓库；<br />           ------多个仓库id之间用逗号分隔；<br />           ------!repol：表示匹配所有仓库，除了repol。<br />           注意：如果配置了多个仓库，首先匹配id精确匹配的镜像，否则maven使用第一个匹配的镜像。
<a name="-proxies-"></a>
## <proxies/>
代理服务器设置
```markup
...
<a name="-profiles-"></a>
## <profiles/>
这里的profile元素是pom.xml的profile元素的一个裁剪版本，它包含activation、repositories、pluginRepositories和properties元素。<br />如果一个在settins中的profile是激活的，它的值将覆盖在一个POM或者profiles.xml文件中的任何相同id的profiles。
